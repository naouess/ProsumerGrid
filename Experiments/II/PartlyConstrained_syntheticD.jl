...
begin
	...
	myPV = PV(ξ = t -> abs.(dd(t)[1]), η_gen = t -> 1., LI = LI(kp = 400., ki = 0.05, T_inv = 1/0.04), ILC = ILC_pars0, M_inv = 1/5.)
	myLoad = Load(ξ = t -> abs.(dd(t)[2]), η_load = t -> 1., LI = LI(kp = 110., ki = 0.004, T_inv = 1/0.045), ILC = ILC_pars1, M_inv = 1/4.8)
	mySlack = Slack(ξ = t -> abs.(dd(t)[3]), η_gen = t -> 1., LI = LI(kp = 100., ki = 0.05, T_inv = 1/0.047), ILC = ILC_pars2, M_inv = 1/4.1)
	myBattery = Battery(η = η(gen=t->1., load=t->1.), LI = LI(kp = 200., ki = 0.001, T_inv = 1/0.043), ILC = ILC_pars3, M_inv = 1/4.8, C=1000000.)
	...
	lines = [StaticEdge(f! = myLine, dim = 1) for i in 1:5]
...
begin
	...
	plot!(0:num_days*l_day, t -> (-abs(dd(t)[1]) .+ abs(dd(t)[2]) .- abs(dd(t)[3])), alpha=0.2,linewidth=3, linestyle=:dot)
