...
begin
	dd = t -> demand_synth(t)
	myPV = PV(ξ = t -> dd(t)[2], η_gen = t -> 1., LI = LI(kp = 500., ki = 0.05, T_inv = 1/0.8), ILC = ILC_pars0, M_inv = 1/5)
	myLoad = Load(ξ = t -> dd(t)[1], η_load = t -> 1., LI = LI( kp = 500., ki = 0.05, T_inv = 1/0.8), ILC = ILC_pars1, M_inv = 1/4.8)
	mySlack = Slack(η_gen = t -> 1., LI = LI(kp = 500., ki = 0.05, T_inv = 1/0.8), ILC = ILC_pars2, M_inv = 1/5)
	myBattery = Battery(η = η(gen=t->1., load=t->1.), LI = LI(kp = 500., ki = 0.05, T_inv = 1/0.8), ILC = ILC_pars3, M_inv = 1/5, C= 310 ./ 50)
...
begin
	...
	# savefig("$dir/plots/IIappend_real_omega.png")
end
...
begin
	...
	# savefig("$dir/plots/IIappend_real_sum_exact.pdf")
end
...
begin
	...
	# savefig("$dir/plots/IIappend_real_sum_hourly.pdf")
end
...
begin
	...
	# savefig("$dir/plots/IIappend_mismatch_secondwise_mismatch.pdf")
end
...
begin
	...
	ylims!(-1,1.)
	# savefig("$dir/plots/IIappend_node1.pdf")
end
...
begin
	...
	p2 = plot()
	ILC_power_hourly_mean_node = vcat(ILC_power[:,:,node]'...)
	plot!(0:3600:num_days*l_day, t -> dd(t)[1], alpha=0.2,linewidth=3, linestyle=:dot)
	plot!(1:3600:24*num_days*3600, LI_exact[1:num_days*24, node],linewidth=3)
	plot!(1:3600:num_days*24*3600, ILC_power_hourly_mean_node[1:num_days*24], xticks = (0:3600*24:num_days*24*3600, string.(0:num_days)), ytickfontsize=10,
	      xtickfontsize=10, legendfontsize=10, linewidth=3, yaxis=("Normed power",font(10)),legend=false, lc =:black, margin=5Plots.mm)
    ylims!(-1,1)
	# savefig("$dir/plots/IIappend_node2.pdf")
end
...
begin
	...
	plot!(1:3600:num_days*24*3600, ILC_power_hourly_mean_node[1:num_days*24], xticks = (1:3600*24:num_days*24*3600+1, string.(0:num_days)), ytickfontsize=10,
	      xtickfontsize=10, legendfontsize=10, linewidth=3, yaxis=("Normed power",font(10)),legend=false, lc =:black, margin=5Plots.mm)
	ylims!(-1,1)
	# savefig("$dir/plots/IIappend_node3.pdf")
end
...
begin
	...
	plot!(1:3600:num_days*24*3600, ILC_power_hourly_mean_node[1:num_days*24], xticks = (0:3600*24:num_days*24*3600, string.(0:num_days)), ytickfontsize=10,
	      xtickfontsize=10, legendfontsize=10, linewidth=3, yaxis=("Normed power",font(10)),legend=false, lc =:black, margin=5Plots.mm)
	ylims!(-1,1)
	# savefig("$dir/plots/IIappend_node4.pdf")
end
