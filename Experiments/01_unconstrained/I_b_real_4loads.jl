...
begin
	...
	# savefig("$dir/plots/demand_real_10days_omega_kappa075.png")
end
...
begin
	...
	savefig("$dir/plots/demand_real_10days_sum_kappa075_exact.pdf")
end

begin
	psum = plot()
	ILC_power_hourly_mean_sum = vcat(ILC_power[:,:,1]'...) .+ vcat(ILC_power[:,:,2]'...) .+ vcat(ILC_power[:,:,3]'...) .+ vcat(ILC_power[:,:,4]'...)
	plot!(0:3600:num_days*l_day, t -> dd(t)[1] + dd(t)[1] + dd(t)[1] + dd(t)[1], alpha=0.2, linewidth=3, linestyle=:dot) #.- dd(t)[2]
	...
	plot!(1:3600:num_days*24*3600,  ILC_power_hourly_mean_sum[1:num_days*24], xticks = (0:3600*24:num_days*24*3600, string.(0:num_days)), ytickfontsize=14,
	               xtickfontsize=18,legend=false, legendfontsize=10, linewidth=3,xaxis=("Days",font(14)),  yaxis=("Normed power",font(14)),lc =:black, margin=5Plots.mm)
	# savefig("$dir/plots/demand_real_10days_sum_kappa075_hourly.pdf")
end
...
mismatch_noCons = (-(sum(sum_LI_n) + sum(sum_ILC_n) + sum_data_load) + (sum(sum_LI_p) + sum(sum_ILC_p) + sum_data_infeed)) / (sum_data_load  + sum_data_infeed)
...
begin
	...
	# savefig("$dir/plots/mismatch_demand_real_10days_kappa075_exact.pdf")
end
