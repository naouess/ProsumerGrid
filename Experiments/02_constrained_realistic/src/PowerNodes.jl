...
function (pv::PV)(dx, x, e_s, e_d, p, t)
	u_ILC = 0 # pv.ILC.current_background_power
	...
	dx[3] = 0 # (- x[2] - pv.LI.ki * x[3]) * pv.LI.T_inv
	dx[4] = u_LI
	dx[5] = abs(u_LI)
...
function (load::Load)(dx, x, e_s, e_d, p, t)
	...
	u_ILC = 0 # load.ILC.current_background_power
	...
	dx[3] = 0 # (- x[2] - load.LI.ki * x[3]) * load.LI.T_inv
	dx[4] = u_LI
	dx[5] = abs(u_LI)
	nothing
end
...
function (ts::ThermalStorage)(dx, x, e_s, e_d, p, t)
	...
	dx[6] = abs(u_LI)
	if F < 0
		dx[5] =  -(u_LI + u_ILC) / ts.η.gen(t) / ts.C / 3600 - ts.a_loss * (x[5] - ts.l_ss)
	else
		dx[5] =  -(u_LI + u_ILC) * ts.η.load(t) / ts.C / 3600 - ts.a_loss * (x[5] - ts.l_ss)
	end
