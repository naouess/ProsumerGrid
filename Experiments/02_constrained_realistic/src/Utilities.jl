...
@inline function integrals(sol, nd, N, num_days, LI_exact, ILC_power)
	...
	#
	# for i in 1:24*num_days
	# 	sum_mismatch = sum_mismatch + abs(mismatch[i]) * 3600
	# end

	# sum_LI_1 = 0.
	# sum_ILC_1 = 0.
	# sum_data_1 = 0.
	# ILC_power_hourly_mean_sum_1 = zeros((num_days+2)*24)
	#
	# for (i,t) in enumerate(sol.t) #sol.t #1:24*num_days+1
	#     if i == 1
	#         delta_t = (sol.t[i+1] - sol.t[i])
	...
	#     end
	#     for j in 1:N
	#         sum_LI_1 = sum_LI_1 + (- KP[j] * sol(t)[indices_ω[j]] + sol(t)[indices_χ[j]]) * delta_t
	#     end
	#     sum_data_1 = sum_data_1 + (dd(t)[1] - dd(t)[2]) * delta_t
	# end
	#
	# for j in 1:N
	#     ILC = vcat(ILC_power[:,:,j]'...)
	#     ILC_power_hourly_mean_sum = ILC_power_hourly_mean_sum .+ ILC
	#     for i in 1:24*num_days
	#         sum_ILC_1 = sum_ILC_1 + ILC[i] * 3600
	#     end
	# end
	#
	# sum_1 = - sum_data_1 + sum_LI_1 + sum_ILC_1

	return sum_LI_n, sum_LI_p, sum_ILC_n, sum_ILC_p, sum_data_load, sum_data_infeed #, sum_1
end

# a, b = control_integral(sol, nd, N, num_days, ILC_power)
# c = a + b
#
# a ./ c * 100
# b ./ c *  100
...
##
# function control_integral_hourly(sol, nd, N, num_days, ILC_power)
# 	sum_LI = zeros(4)
# 	sum_ILC = zeros(4)
#
# 	indices_ω = idx_containing(nd, :ω)
...
# 	for i in 1:24*num_days+1
# 		for j in 1:N
# 			sum_LI[j] += abs(- KP[j] * sol((i-1)*3600)[indices_ω[j]] + sol((i-1)*3600)[indices_χ[j]])
# 		end
# 	end
#
# 	for j in 1:N
