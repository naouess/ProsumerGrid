...
begin
	...
	myBattery = Battery(η = η(gen=t->1., load=t->1.), LI = LI(kp = 200., ki = 0.001, T_inv = 1/0.043), ILC = ILC_pars3, M_inv = 1/4.8, C= 500 ./ 50, σ = 0) #.00004) # C in kWh, normed # 4.8
...
begin
	...
	cb = CallbackSet(PeriodicCallback(HourlyUpdate, l_hour), #initial_affect= false),
					 PeriodicCallback(DailyUpdate, l_day),#, initial_affect= false))#,
					 )

	# Define the ODE-Problem
	ode_problem = ODEProblem(nd, i0, tspan, callback = cb)#, dt=900, adaptive=false)#, dtmax=60.)
end
...
LI_exact_solt = zeros(num_days*24*3600, N) #length(sol.t),N) #num_days*24*3600, N)
data_solt = zeros(num_days*24*3600) #(length(sol.t)) #
ILC_power_solt = zeros(num_days*24*3600)#(length(sol.t)) #
for i in 1:num_days*24*3600 #enumerate(sol.t) #1:num_days*24*3600
	for j in 1:N
...
# plot(ILC_power_solt)
# plot!(data_solt)
# plot!(LI_exact_solt[:, 1] + LI_exact_solt[:, 2] + LI_exact_solt[:, 3] + LI_exact_solt[:, 4])
# plot(control_solt)
# plot!(data_solt)
...
begin
	...
	# savefig("$dir/plots/VI_allILC_layout4Nodes.pdf")
end
